...
def teacher_tab_take_attendance():
    ...
    with c2:    
        if st.button("Run Face Scan",width='stretch',type='secondary',icon=':material/analytics:',disabled=not has_photos):
...
def teacher_tab_attendance_records():
    ...
    display_df=(summary.sort_values(by='ts_group',ascending=False)[['Time','Subject','Subject_Code','Attendance Stats']])
...
def teacher_screen_login():
    ...
    st.space()
    teacher_username=st.text_input("Enter UserName",placeholder='E.g. Sidhartha')
...
def register_teacher(teacher_username,teacher_name,teacher_password,teacher_pass_conf):
    if not teacher_username or not teacher_name or not teacher_password or not teacher_pass_conf:
        return False,"All Fields Are Required"
    if check_teacher_exists(teacher_username):
        return False,"Username Already Taken"
    if teacher_password!=teacher_pass_conf:
...
def teacher_screen_register():
    ...
    with c1:
        header_dashboard()
    with c2:
        if st.button("Go Back To Home",type='secondary',key='loginbackhomebtn'):
            st.session_state['login_type']=None
            st.rerun()
    ...
    st.space()
    teacher_username=st.text_input("Enter UserName",placeholder='E.g. Sidhartha')
    teacher_name=st.text_input("Enter Name",placeholder="E.g. Y Sidhartha")
    teacher_password=st.text_input("Enter Password",type='password',placeholder='Enter Your Password')
    teacher_pass_conf=st.text_input("Confirm Password",type='password',placeholder="Confirm Your Password")
    st.divider()
    col1,col2=st.columns(2)
    with col1:
        if st.button('Register Now',icon=':material/passkey:',width='stretch'):
            success,message=register_teacher(teacher_username,teacher_name,teacher_password,teacher_pass_conf)
            if success:
                st.success(message)
                import time
                time.sleep(1)
